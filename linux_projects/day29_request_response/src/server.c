...
static int read_message(int fd,char *buffer,size_t buffer_size){
    ...
    ssize_t n=read_exact(fd,&net_length,sizeof(net_length));
    if(n<=0){
        return (int)n;
    }
...
static int send_message(int fd,const char *message){
    ...
    ssize_t n=send_exact(fd,&net_length,sizeof(net_length));
    if(n!=sizeof(net_length)){
        return -1;
    }
    n=send_exact(fd,message,length);
    if(n!=length){
        return -1;
    }
    return 0;
...
int main(void){
    ...
    if(server_fd<0){
     ...
     printf("request-response server listening on port %d\n", SERVER_PORT);
